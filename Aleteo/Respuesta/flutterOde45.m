%flutterOde45.m
%% Programa para obtener los desplazamientos para 8 derivadas aerodinÃ¡micas
clear
close all
proDim = 9;
derAer = 8;
...
df = 'flut8a45';
tic
[t45,x45] = ode45(df,[t0 tf],x0,[],constHA,constB,velU,velW,dh);
toc
maxH=abs(max(x45(:,1)));
maxA=abs(max(x45(:,2)));
sH=std(x45(:,1));
...
fprintf(archivo,'Rotación máxima : %6.4f rad\r\n',maxA);
fprintf(archivo,'Desviación estándar horizontal: %6.4f m\r\n',sH);
fprintf(archivo,'Desviacion estámdar rotacional: %6.4f rad\r\n',sA);
